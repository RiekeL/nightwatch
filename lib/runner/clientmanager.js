...
ClientManager.prototype.init = function(opts) {
  ...
  this['@client'].once('selenium:session_create', function() {
    console.log(this.api.capabilities)
    var capabilities = this.api.capabilities || {};
    var browserName = (capabilities.browserName && capabilities.browserName.toUpperCase().replace(' ','_')) || '';
    var browserVersion = capabilities.browserVersion || capabilities.version ||  '';
    var platformName = capabilities.platformName || '';
    var platformVersion = capabilities.platformVersion || capabilities.platform || '';
    var mobileDevice = capabilities.mobile ? capabilities.mobile.version.replace(' ','_')+'_' : '';
    self.options.report_prefix = mobileDevice + browserName + '_' + browserVersion + '_' + platformName +'_' + platformVersion + '_';
  });
  return this;
};
