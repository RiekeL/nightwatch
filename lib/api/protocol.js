...
module.exports = function(Nightwatch) {
   ...
   *
   * @link /session/:sessionId/touch/down
   * @param {number} xoffset X offset to move to.
   * @param {number} yoffset Y offset to move to.
   * @param {function} [callback] Optional callback function to be called when the command finishes.
   * @api protocol.useractions
   */
  Actions.touchDown = function(xoffset, yoffset, callback) {
    var data = {};
    if (typeof xoffset == 'number') {
      data.x = parseInt(xoffset);
    }
    if (typeof yoffset == 'number') {
      data.y = parseInt(yoffset);
    }
    return postRequest('/touch/down', data, callback);
  };
  
  /**
   * Finger move on the screen.
   *
   * @link /session/:sessionId/touch/move
   * @param {number} xoffset X coordinate on the screen.
   * @param {number} yoffset Y coordinate on the screen.
   * @param {function} [callback] Optional callback function to be called when the command finishes.
   * @api protocol.useractions
   */
  Actions.touchMove = function(xoffset, yoffset, callback) {
    var data = {};
    if (typeof xoffset == 'number') {
      data.x = parseInt(xoffset);
    }
    ...
    return postRequest('/touch/move', data, callback);
  };

  /**
   * Finger up on the screen.
   *
   * @link /session/:sessionId/touch/up
   * @param {number} xoffset X coordinate on the screen.
   * @param {number} yoffset Y coordinate on the screen.
   * @param {function} [callback] Optional callback function to be called when the command finishes.
   * @api protocol.useractions
   */
  Actions.touchUp = function(xoffset, yoffset, callback) {
    var data = {};
    if (typeof xoffset == 'number') {
      data.x = parseInt(xoffset);
    }
    ...
    return postRequest('/touch/up', data, callback);
  };
   
  /**
    * Performs a specific touch actions. The action object is an array and its
    ...
    * @link: /session/${this.client.sessionId}/touch/perform
    * @param {Array} Array with action objects to perform.
    * @param {function} [callback] Optional callback function to be called when the command finishes.
    * @api protocol.useractions
  */
  Actions.touchPerform = function( actions, callback){
      return postRequest('/touch/perform', {actions});       
  };

  /**
   * Perform action
   * @link: /session/${this.client.sessionId}/actions
   * @param {number} x X coordinate to start touch on the screen.
   * @param {number} y Y coordinate to start touch on the screen.
   * @param {number} x2 X coordinate to end touch on the screen.
   * @param {number} y2 Y coordinate to end touchon the screen.
   * @param {function} [callback] Optional callback function to be called when the command finishes.
   * @api protocol.useractions
   */
  Actions.actions = function( actions, callback){
    
      return postRequest('/actions', { actions });       
  };
